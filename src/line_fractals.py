...
def draw_koch_snowflake(surface, clock, fps):
    ...
    _draw_koch_line(surface, clock, fps, snowflake_top, snowflake_left)
    _draw_koch_line(surface, clock, fps, snowflake_left, snowflake_right)
    _draw_koch_line(surface, clock, fps, snowflake_right, snowflake_top)
...
def draw_fractal_tree(surface, clock, fps):
    ...
    _draw_tree_line(surface, clock, fps, start, -90, 9)


def _draw_koch_line(surf, clock, fps, start, end):
    if (end - start).magnitude() / 3 < MIN_LINE_LENGTH:
        ...
        _draw_koch_line(surf, clock, fps, start, left)
        _draw_koch_line(surf, clock, fps, left, top)
        _draw_koch_line(surf, clock, fps, top, right)
        _draw_koch_line(surf, clock, fps, right, end)


def _draw_tree_line(surf, clk, fps, start, angle, depth):
    if depth > 0:
        end = pygame.Vector2(
        ...
        _draw_tree_line(surf, clk, fps, end, angle - 30, depth - 1)
        _draw_tree_line(surf, clk, fps, end, angle + 30, depth - 1)
